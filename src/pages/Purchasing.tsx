...
export function Purchasing({ currentUser }: PurchasingProps = {}) {
  ...
  const generatePOPrintHTML = (order: PurchaseOrder) => {
    const shippingFee = Math.max(0, Number((order as any).transportation_fee ?? (order as any).transportationFee ?? order.shipping_cost ?? (order as any).delivery_fee ?? 0));
  ...
  <tr>
    ...
    const paymentMethod = ((order as any).settlement_mode || (order as any).settlementMode || order.payment_method || (order as any).paymentMethod || (order as any).settlement_type || 'CREDIT').toUpperCase();
  ...
  const downloadPO_PDF = (order: PurchaseOrder) => {
    ...
    if (['received', 'completed', 'paid'].includes((order.status || '').toLowerCase())) {
      const receivedDate = order.received_at || (order as any).received_date || (order as any).updated_at 
        ? new Date(order.received_at || (order as any).received_date || (order as any).updated_at).toLocaleDateString() 
        : 'N/A';
      const receivedBy = order.received_by || (order as any).received_by_name || currentUser?.name || 'Staff';
                  ...
                  {orders.map((order) => (
                      ...
                      <td className="px-6 py-4 text-center">
                          <div className="flex gap-2 justify-center items-center">
                            ...
                            {['received', 'completed', 'paid'].includes((order.status || '').toLowerCase()) && (
