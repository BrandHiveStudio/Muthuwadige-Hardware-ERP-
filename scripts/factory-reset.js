...
const OPERATIONAL_TABLES_TO_WIPE = [
  ...
  // 5. Drawer & Shift Records
  'shift_logs',

  // 6. Queues & System Logs
  'sync_queue',
  'sync_pull_marker',
  'audit_logs',
  'backup_logs',
...
async function resetSqliteDatabase(dbPath, label) {
  ...
  try {
    ...
    // 2. Wipe operational & transaction tables
    ...
    // 3. Clear shift opening floats stored in app_settings / system_settings
    try {
      await db.run("DELETE FROM app_settings WHERE key LIKE 'OPENING_FLOAT_%'");
      console.log(`  ✓ Cleared shift opening floats from app_settings`);
    } catch (err) {
      // Table might not exist
    }

    try {
      await db.run("DELETE FROM system_settings WHERE key LIKE 'OPENING_FLOAT_%'");
      console.log(`  ✓ Cleared shift opening floats from system_settings`);
    } catch (err) {
      // Ignore
    }

    // 4. Reset item stock quantities to 0
    try {
      const prodCols = await db.all('PRAGMA table_info(products)');
      const colNames = new Set(prodCols.map(c => c.name));
      const setClauses = [];
      if (colNames.has('stock')) setClauses.push('stock = 0');
      if (colNames.has('stock_quantity')) setClauses.push('stock_quantity = 0');
      if (colNames.has('quantity')) setClauses.push('quantity = 0');

      if (setClauses.length > 0) {
        await db.run(`UPDATE products SET ${setClauses.join(', ')}`);
        console.log(`  ✓ Reset product stock quantities to 0 (${setClauses.join(', ')})`);
      }
    } catch (err) {
      console.warn('  ! Product stock reset notice:', err.message);
    }

    // 5. Reset customer credit balances & purchase history
    try {
    ...
    // 6. Reset supplier payable/outstanding balances
    try {
    ...
    // 7. Reset sequence auto-increment counters
    try {
    ...
    // 8. Reset system settings sync status & invoice sequence
    try {
      await db.run(
    ...
    // 9. Final sweep: Clear any audit logs or sync queue entries generated by update triggers
    ...
    // 10. Maintenance: WAL checkpoint & VACUUM
    try {
...
async function resetTursoCloudDatabase() {
  ...
  try {
    ...
    // 2. Clear opening floats from app_settings / system_settings
    try {
      await txn.execute("DELETE FROM app_settings WHERE key LIKE 'OPENING_FLOAT_%'");
      console.log(`  ✓ Cleared remote shift opening floats from app_settings`);
    } catch (err) {
      // Ignore
    }

    try {
      await txn.execute("DELETE FROM system_settings WHERE key LIKE 'OPENING_FLOAT_%'");
      console.log(`  ✓ Cleared remote shift opening floats from system_settings`);
    } catch (err) {
      // Ignore
    }

    // 3. Reset product stock quantities to 0
    try {
      await txn.execute('UPDATE products SET stock = 0, stock_quantity = 0 WHERE 1=1');
      console.log(`  ✓ Reset remote product stock quantities to 0`);
    } catch (err) {
      try {
        await txn.execute('UPDATE products SET stock = 0 WHERE 1=1');
        console.log(`  ✓ Reset remote product stock to 0`);
      } catch (e) {
        console.warn('  ! Remote product stock reset notice:', e.message);
      }
    }

    // 4. Reset customer credit balances
    try {
      await txn.execute(
    ...
    // 5. Reset supplier payable balances
    try {
    ...
    // 8. Final sweep on Turso: Clear any audit logs or sync queue entries generated by update triggers
...
async function printVerificationSummary(dbOrClient, label, isTurso = false) {
  ...
  const shiftLogsCount = await queryCount('SELECT COUNT(*) as c FROM shift_logs');
  ...
  const totalStock = await queryCount('SELECT COALESCE(SUM(stock), 0) as s FROM products');
  ...
  const userCount = await queryCount('SELECT COUNT(*) as c FROM users');

  console.log(`\n  📊 [${label}] Post-Reset Verification:`);
  console.log(`     - Sales / Invoices:           ${salesCount} (Expected: 0)`);
  console.log(`     - Shift Logs & Drawer State:  ${shiftLogsCount} (Expected: 0)`);
  ...
  console.log(`     - Total Active Product Stock: ${totalStock} units (Expected: 0)`);
  console.log(`     --------------------------------------------------`);
  ...
  console.log(`     - User Accounts:              ${userCount} preserved`);
}
...
async function main() {
  console.log('🚀 Initiating Full Production Factory Reset to Zero Data...');
